    ...
    <main className="site-shell">
      ...
      <section className="calculator-card" aria-labelledby="calculator-title">
        ...
        <section className="result" aria-live="polite" aria-label="Paint calculation result">
          {validResult ? <>
            <p className="result-label">Paint needed:</p>
            <output className="gallons">{formatNumber(result.gallonsNeeded)} <span>gallons</span></output>
            <div className="order"><span>Recommended order</span><strong>{formatNumber(result.recommendedGallons)} gallons</strong><small>Includes a 10% planning allowance for touch-ups, waste, and normal variation.</small></div>
            <p className="area">Paintable area: <strong>{formatNumber(result.paintArea)} sq ft</strong></p>
            <div className="actions"><button className="secondary-button" type="button" onClick={copy}>{copyStatus === 'success' ? 'Copied!' : 'Copy Results'}</button><button className="text-button" type="button" onClick={reset}>Reset</button></div>
            {copyStatus && <p className="copy-status" role="status">{copyStatus === 'success' ? 'Results copied.' : 'Copy unavailable. Please try again.'}</p>}
          </> : <p className="empty-result">Your estimate will appear here.</p>}
        </section>
      ...
      <article className="content">
        ...
        <section><h2>FAQ</h2>
          <details><summary>What coverage should I use?</summary><p>Use the coverage printed on your paint can. A common planning range is roughly 350–400 square feet per gallon, but products vary.</p></details>
          <details><summary>Does this account for doors and windows?</summary><p>Yes. The calculator uses approximate areas of 21 square feet per door and 15 square feet per window.</p></details>
          <details><summary>Does this calculate primer too?</summary><p>No. This calculator estimates paint quantity. If you need primer, calculate it separately based on the primer's coverage and the surface being primed.</p></details>
        </section>
      </article>
      <footer><span>Free paint calculator for practical project planning.</span>{' '}<a href="/privacy.html">Privacy Policy</a></footer>
    </main>
  )
}
